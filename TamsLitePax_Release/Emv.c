#include		<posapi.h>
#include		<posapi_all.h>
#include		"Applib.h"
#include		"emv.h"
#include		"debug.h"
#include		"emvlib.h"
#include		"EmvApplist.h"
#include        "EmvCaKey.h"
#include        "TamsLite.h"
#include		"Security.h"
#include		"MagStripe.h"
#include        "Purchase.h"
#include		"Bin2Hex.h"
#include		"Bin2Ascii.h"
#include		"Printer.h"
#include		"EftUtil.h"
EMV_PARAM gxEmvParam;
char gszAmount[12+1]={0,};
unsigned char aucAmount[6+1]={0,};
EftTransaction MyEmvTransaction = {0};
int cEMVUnknowTLVData(ushort iTag, uchar *psDat, int iDataLen)
{
	//LOG_PRINTF(("cEMVUnknowTLVData() iTag %d",iTag));

	switch( iTag )
	{
		/*
		'C' = CASH DESBUS
		'Z' = ATM CASH
		'O' = COLLEGE SCHOOL
		'H' = HOTEL/SHIP
		'X' = TRANSFER
		'A' = AUTO MOBILE/RENTAL
		'F' = RESTAURANT
		'T' = TELEPHONE ORDER PREAUTH
		'U' = UNIQUE TRANS
		'R' = RENTAL/OTHER TRANS
		*/
	case 0x9F53:		// Transaction Category Code (TCC) - Master Card
		*psDat = 'R';	// 0x52;
		break;

	default:
		return -1;
	}

	return EMV_OK;
}

void cEMVVerifyPINOK(void)
{
	//LOG_PRINTF(("cEMVVerifyPINOK()"));
	ScrCls();
	ScrFontSet(0);
	ScrPrint(0*6, 2,ASCII,"%s","PIN OK!");
    DelayMs(1000);
}

int cCertVerify(void)
{
	EMV_MCKPARAM stMckParam;
	//LOG_PRINTF(("cCertVerify()"));
	
	stMckParam.ucBypassPin = 1;
	stMckParam.ucBatchCapture = 1;
	EMVSetMCKParam(&stMckParam);
	
	return -1;
}

int cEMVGetHolderPwd(int iTryFlag, int iRemainCnt, uchar *pszPlainPin)
{
    // Implement UI here for allowing user input PIN
	
	//LOG_PRINTF(("cEMVGetHolderPwd() iTryFlag %d iRemainCnt %d",iTryFlag,iRemainCnt));
	if(pszPlainPin!=NULL)
	{
		//LOG_PRINTF(("cEMVGetHolderPwd() -> Pin Offline!!!"));
		//LOG_PRINTF(("cEMVGetHolderPwd() pszPlainPin[0] %d",pszPlainPin[0]));
		//sprintf(MyEmvTransaction.PinBlock,"%s","OFFLINE");
		
		return EMV_OK;
	}
	else
	{
		//LOG_PRINTF(("cEMVGetHolderPwd () ->Pin Online!!!"));
		if(MyDevice.PinBlockEncryptionType == TRIDES)
		{
			unsigned char TrackTwoString[200] = {0};
			unsigned char HexTrack2Data[50] = {0};
			int TrackTwoLen;
			TrackTwo OutPut;
			memset(&OutPut,0,sizeof(OutPut));
			EMVGetTLVData(tagTRACK2_EQUIVALENT_DATA,TrackTwoString,&TrackTwoLen);
			Bin2Ascii(TrackTwoString,TrackTwoLen,HexTrack2Data);
			ParseTrackTwo((char *)HexTrack2Data,&OutPut,1);
			TriDesGetPin(MyEmvTransaction.PinBlock, OutPut.Pan, "ENTER PIN");
		}else
		{
			//DUKPT
		}
		return EMV_OK;
	}
}

int cEMVWaitAppSel(int TryCnt, EMV_APPLIST List[], int AppNum)
{
    // Implement UI here for allowing user select/confirm EMV application of the card.
	/*
	int i = 0;
	int SelectedIndex = 0;
	PubShowTitle(TRUE,(unsigned char *)"SELECT APP");
	if(AppNum  <= 3)
	{
		int Spacer = 2;
		char Holder[400] = {0};
		
		for(i = 0;i<=AppNum-1;i++)
		{
			memset(Holder,0,sizeof(Holder));
			strcpy(Holder,(char *)List[i].AppName);
			ScrGotoxy(0, Spacer);
			Lcdprintf(Holder); 
			Spacer+=2;
		}
		switch(getkey())
		{
		case KEYATM2:
			return 0;
			break;
		case KEYATM3:
			return 1;
			break;
		case KEYATM4:
			return 2;
			break;
		}
	}
	
	//LOG_PRINTF(("cEMVWaitAppSel()"));

            ScrCls();
            ScrPrint(0,0,0,"(EMV APP SELECT)");
            DelayMs(2000);
            ScrCls();
            ScrPrint(0,0,0,"PROCESSING...");

	return SelectedIndex;
	*/
	return 0;
}

void PromptRemoveICC(void)
{
	
	IccClose(0);
	if( IccDetect(0)!=0 )
	{
		return;
	}
	
	ScrCls();
	ScrFontSet(0);
	ScrPrint(0*6, 2,ASCII,"%s","Remove Card!");

	
	while( IccDetect(0)==0 )
	{
		Beep();
		DelayMs(500);
	}
}

void EraseExpireCAPK(void)
{
	int			iRet, iCnt;
	EMV_CAPK	stCAPK;
	
	//LOG_PRINTF(("EraseExpireCAPK()"));
	
	for(iCnt=0; iCnt<MAX_KEY_NUM; iCnt++)
	{
		memset(&stCAPK, 0, sizeof(EMV_CAPK));
		iRet = EMVCheckCAPK(&stCAPK.KeyID, stCAPK.RID);
		if( iRet==EMV_OK )
		{
			break;
		}
		EMVDelCAPK(stCAPK.KeyID, stCAPK.RID);
	}
}

void ShowTVR_TAC_IAC(void)
{
	int iLength,iCnt,iRet;
	unsigned char aucBuffer[5+1];
	EMV_APPLIST		stEmvApp;
	uchar			sTermAID[16];

	memset(aucBuffer,0,sizeof(aucBuffer));
	EMVGetTLVData(0x95,   aucBuffer,  &iLength);
	//LOG_HEX_PRINTF("ShowTVR_TAC_IAC TVR",aucBuffer,iLength);

	memset(aucBuffer,0,sizeof(aucBuffer));
	EMVGetTLVData(0x9B,   aucBuffer,  &iLength);
	//LOG_HEX_PRINTF("ShowTVR_TAC_IAC TSI",aucBuffer,iLength);
	
	memset(aucBuffer,0,sizeof(aucBuffer));
	EMVGetTLVData(0x9F0E, aucBuffer, &iLength);
	//LOG_HEX_PRINTF("ShowTVR_TAC_IAC IAC Denial",aucBuffer,iLength);

	memset(aucBuffer,0,sizeof(aucBuffer));
	EMVGetTLVData(0x9F0D, aucBuffer, &iLength);
	//LOG_HEX_PRINTF("ShowTVR_TAC_IAC IAC Online",aucBuffer,iLength);

	memset(aucBuffer,0,sizeof(aucBuffer));
	EMVGetTLVData(0x9F0F, aucBuffer, &iLength);
	//LOG_HEX_PRINTF("ShowTVR_TAC_IAC IAC Default",aucBuffer,iLength);

	// search TAC from terminal parameter
	memset(sTermAID, 0, sizeof(sTermAID));
	EMVGetTLVData(0x9F06, sTermAID, &iLength);
	for(iCnt=0; iCnt<MAX_APP_NUM; iCnt++)
	{
		memset(&stEmvApp, 0, sizeof(EMV_APPLIST));
		iRet = EMVGetApp(iCnt, &stEmvApp);
		if( iRet!=EMV_OK )
		{
			continue;
		}
		if( memcmp(sTermAID, stEmvApp.AID, stEmvApp.AidLen)==0 )
		{
			//LOG_HEX_PRINTF("ShowTVR_TAC_IAC stEmvApp.TACDenial",stEmvApp.TACDenial,5);
			//LOG_HEX_PRINTF("ShowTVR_TAC_IAC stEmvApp.TACOnline",stEmvApp.TACOnline,5);
			//LOG_HEX_PRINTF("ShowTVR_TAC_IAC stEmvApp.TACDefault",stEmvApp.TACDefault,5);
			break;
		}
	}
	
	return;
}

int cEMVSetParam(void)
{
    // Implement only when necessary

	//LOG_PRINTF(("cEMVSetParam()"));
	
	return EMV_OK;
}

int cEMVInputAmount(unsigned long *AuthAmt, ulong *CashBackAmt)
{
    // IMplement UI for allowing user to input transaction amount
	
	//LOG_PRINTF(("cEMVInputAmount()  AuthAmt %ld gszAmount %s",*AuthAmt,gszAmount));

           /* ScrCls();
            ScrPrint(0,0,0,"(INPUT AMOUNT)");
            DelayMs(2000);
            ScrCls();
            ScrPrint(0,0,0,"PROCESSING...");*/
	
	if( gszAmount!=0 )
	{
		*AuthAmt = atol((char *)gszAmount);
		if( CashBackAmt!=NULL )
		{
			*CashBackAmt = 0L;
		}
	}
	else
	{
		*AuthAmt = 0L;
		if( CashBackAmt!=NULL )
		{
			*CashBackAmt = 0L;
		}
	}
	
	if( CashBackAmt==NULL )
	{
        // Disabled in EMV 4xx
        /*
		EMVSetTLVData(0x9F03, (uchar *)"\x00\x00\x00\x00\x00\x00", 6);
		EMVSetTLVData(0x9F04, (uchar *)"\x00\x00\x00\x00", 4);
        */
	}
	
	return EMV_OK;
}

void cEMVAdviceProc(void)
{
	//LOG_PRINTF(("cEMVAdviceProc"));
	
	return ;
}

int cEMVReferProc(void)
{
	//LOG_PRINTF(("cEMVReferProc"));
	
	return 0;
}

int  cEMVOnlineProc(uchar *psRspCode,  uchar *psAuthCode, int *piAuthCodeLen,
					uchar *psAuthData, int *piAuthDataLen,
					uchar *psScript,   int *piScriptLen)
{
    // Implement online processing here.
	
	TrackTwo OutPut;
	unsigned char Cryptogram[50] = {0};
	unsigned char CryptogramInformationData[50] = {0};
	unsigned char IssuerApplicationData[50] = {0};
	unsigned char UnpredictableNumber[50] = {0};
	unsigned char ApplicationTransactionCounter[50] = {0};
	unsigned char TerminalVerificationResult[50] = {0};
	unsigned char TransactionDate[50] = {0};
	unsigned char TransactionType[50] = {0};
	unsigned char AmountAuthorization[50] = {0};
	unsigned char TransactionCurrencyCode[50] = {0};
	unsigned char Reserved01[50] = "";
	unsigned char ApplicationInterchangeProfile[50] = {0};
	unsigned char TerminalCountryCode[50] = {0};
	unsigned char AmountOther[50] = {0};
	unsigned char TerminalCapabilities[50] = {0};
	unsigned char ApplicationIdentication[50] = {0};
	unsigned char Reserved02[50] = "";
	unsigned char CardAuthenticatorResultCode[50] = {0};
	unsigned char TerminalType[50] = {0};
	unsigned char InterfaceDeviceSerialNumber[50] = {0};
	unsigned char TransactionCategory[50] = {0};
	unsigned char DedicatedFileName[50] = {0};// (Optional)
	unsigned char TerminalIdentificationVersionNumber[50] = {0};
	unsigned char TransactionSequenceNumber[50] = {0};// (Optional)
	unsigned char CVMResult[50] = {0};
	unsigned char Track2Data[50] = {0};
	unsigned char Temp[9] = {0};
	int CryptogramLen,
		CryptogramInformationDataLen,
		IssuerApplicationDataLen,
		UnpredictableNumberLen,
		ApplicationTransactionCounterLen,
		TerminalVerificationResultLen,
		TransactionDateLen,
		TransactionTypeLen,
		AmountAuthorizationLen,
		TransactionCurrencyCodeLen,
		ApplicationInterchangeProfileLen,
		TerminalCountryCodeLen,
		AmountOtherLen,
		TerminalCapabilitiesLen,
		ApplicationIdenticationLen,
		CardAuthenticatorResultCodeLen,
		TerminalTypeLen,
		InterfaceDeviceSerialNumberLen,
		TransactionCategoryLen,
		DedicatedFileNameLen,
		TerminalIdentificationVersionNumberLen,
		TransactionSequenceNumberLen,
		CVMResultLen,
		Track2DataLen,
		iRet,
		ATC
		;
	
	unsigned char	HexCryptogram[50] = {0};
	unsigned char	HexCryptogramInformationData[50]={0};
	unsigned char   HexIssuerApplicationData[50]={0};
	unsigned char   HexUnpredictableNumber[50]={0};
	unsigned char   HexApplicationTransactionCounter[50]={0};
	unsigned char   HexTerminalVerificationResult[50]={0};
	unsigned char   HexTransactionDate[50]={0};
	unsigned char   HexTransactionType[50]={0};
	unsigned char   HexAmountAuthorization[50]={0};
	unsigned char   HexTransactionCurrencyCode[50]={0};
	unsigned char   HexApplicationInterchangeProfile[50]={0};
	unsigned char   HexTerminalCountryCode[50]={0};
	unsigned char   HexAmountOther[50]={0};
	unsigned char   HexTerminalCapabilities[50]={0};
	unsigned char   HexApplicationIdentication[50]={0};
	unsigned char   HexCardAuthenticatorResultCode[50]={0};
	unsigned char   HexTerminalType[50]={0};
	unsigned char   HexInterfaceDeviceSerialNumber[50]={0};
	unsigned char   HexTransactionCategory[50]={0};
	unsigned char   HexDedicatedFileName[50]={0};
	unsigned char   HexTerminalIdentificationVersionNumber[50]={0};
	unsigned char   HexTransactionSequenceNumber[50]={0};
	unsigned char   HexCVMResult[50]={0};
	unsigned char   HexTrack2Data[50]={0};
			char IDDDataString[500] = {0};
	
	ScrCls();      
	ScrPrint(0,0,0,"ONLINE PROCESS...");
    DelayMs(2000);
    ScrCls();
    ScrPrint(0,0,0,"PROCESSINGB...");

	EMVGetTLVData(tagAPPL_CRYPTOGRAM,Cryptogram,&CryptogramLen);
	EMVGetTLVData(tagCRYPTOGRAM_INFO,CryptogramInformationData,&CryptogramInformationDataLen);
	EMVGetTLVData(tagISSUER_APPL_DATA,IssuerApplicationData,&IssuerApplicationDataLen);
	EMVGetTLVData(tagUNPREDICTABLE_NUMBER,UnpredictableNumber,&UnpredictableNumberLen);
	EMVGetTLVData(tagATC,ApplicationTransactionCounter,&ApplicationTransactionCounterLen);
	EMVGetTLVData(tagTVR,TerminalVerificationResult,&TerminalVerificationResultLen);
	EMVGetTLVData(tagTRAN_DATE,TransactionDate,&TransactionDateLen);
	EMVGetTLVData(tagTRAN_TYPE,TransactionType,&TransactionTypeLen);
	EMVGetTLVData(tagAUTH_AMOUNT_NUM,AmountAuthorization,&AmountAuthorizationLen);
	EMVGetTLVData(tagTRAN_CURR_CODE,TransactionCurrencyCode,&TransactionCurrencyCodeLen);
	EMVGetTLVData(tagAIP,ApplicationInterchangeProfile,&ApplicationInterchangeProfileLen);
	EMVGetTLVData(tagTERML_COUNTRY_CODE,TerminalCountryCode,&TerminalCountryCodeLen);
	//EMVGetTLVData(tagOTHER_AMOUNT_NUM,AmountOther,&AmountOtherLen);
	EMVGetTLVData(tagTERML_CAPABILITIES,TerminalCapabilities,&TerminalCapabilitiesLen);
	EMVGetTLVData(tagAID_CARD,ApplicationIdentication,&ApplicationIdenticationLen);
	EMVGetTLVData(tagCVM_RESULT,CardAuthenticatorResultCode,&CardAuthenticatorResultCodeLen);
	EMVGetTLVData(tagTERML_TYPE,TerminalType,&TerminalTypeLen);
	EMVGetTLVData(tagIFD_SERIAL_NUM,InterfaceDeviceSerialNumber,&InterfaceDeviceSerialNumberLen);
	EMVGetTLVData(tagTRN_CATEGORY_CODE,TransactionCategory,&TransactionCategoryLen);
	EMVGetTLVData(tagDF_NAME,DedicatedFileName,&DedicatedFileNameLen);
	EMVGetTLVData(tagTERML_ID,TerminalIdentificationVersionNumber,&TerminalIdentificationVersionNumberLen);
	EMVGetTLVData(tagTRAN_SEQUENCE_COUNTER,TransactionSequenceNumber,&TransactionSequenceNumberLen);
	EMVGetTLVData(tagCVM_RESULT,CVMResult,&CVMResultLen);
	EMVGetTLVData(tagTRACK2_EQUIVALENT_DATA,Track2Data,&Track2DataLen);	
	
	
	bin2ascii(Cryptogram,CryptogramLen,(char *)HexCryptogram);
	bin2ascii(CryptogramInformationData,CryptogramInformationDataLen,(char *)HexCryptogramInformationData);
	bin2ascii(IssuerApplicationData,IssuerApplicationDataLen,(char *)HexIssuerApplicationData);
	bin2ascii(UnpredictableNumber,UnpredictableNumberLen,(char *)HexUnpredictableNumber);
	bin2ascii(ApplicationTransactionCounter,ApplicationTransactionCounterLen,(char *)HexApplicationTransactionCounter);
	bin2ascii(TerminalVerificationResult,TerminalVerificationResultLen,(char *)HexTerminalVerificationResult);
	bin2ascii(TransactionDate,TransactionDateLen,(char *)HexTransactionDate);
	bin2ascii(TransactionType,TransactionTypeLen,(char *)HexTransactionType);
	bin2ascii(AmountAuthorization,AmountAuthorizationLen,(char *)HexAmountAuthorization);
	bin2ascii(TransactionCurrencyCode,TransactionCurrencyCodeLen,(char *)HexTransactionCurrencyCode);
	bin2ascii(ApplicationInterchangeProfile,ApplicationInterchangeProfileLen,(char *)HexApplicationInterchangeProfile);
	bin2ascii(TerminalCountryCode,TerminalCountryCodeLen,(char *)HexTerminalCountryCode);
	//Bin2Ascii(AmountOther,AmountOtherLen,HexAmountOther);
	bin2ascii(TerminalCapabilities,TerminalCapabilitiesLen,(char *)HexTerminalCapabilities);
	bin2ascii(ApplicationIdentication,ApplicationIdenticationLen,(char *)HexApplicationIdentication);
	bin2ascii(CardAuthenticatorResultCode,CardAuthenticatorResultCodeLen,(char *)HexCardAuthenticatorResultCode);
	bin2ascii(TerminalType,TerminalTypeLen,(char *)HexTerminalType);
	bin2ascii(InterfaceDeviceSerialNumber,InterfaceDeviceSerialNumberLen,(char *)HexInterfaceDeviceSerialNumber);
	bin2ascii(TransactionCategory,TransactionCategoryLen,(char *)HexTransactionCategory);
	bin2ascii(DedicatedFileName,DedicatedFileNameLen,(char *)HexDedicatedFileName);
	bin2ascii(TerminalIdentificationVersionNumber,TerminalIdentificationVersionNumberLen,(char *)HexTerminalIdentificationVersionNumber);
	bin2ascii(TransactionSequenceNumber,TransactionSequenceNumberLen,(char *)HexTransactionSequenceNumber);
	bin2ascii(CVMResult,CVMResultLen,(char *)HexCVMResult);

	bin2ascii(Track2Data,Track2DataLen,(char *)HexTrack2Data);

	

	sprintf((char *)HexAmountOther,"%s","000000000000");
	
	sprintf(IDDDataString,"%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%d|%s|",
								HexCryptogram,
								HexCryptogramInformationData,
								HexIssuerApplicationData,
								HexUnpredictableNumber,
								HexApplicationTransactionCounter,
								HexTerminalVerificationResult,
								HexTransactionDate,
								HexTransactionType,
								HexAmountAuthorization,
								HexTransactionCurrencyCode,
								"00",//Reserved01,
								HexApplicationInterchangeProfile,
								HexTerminalCountryCode,
								HexAmountOther,
								HexTerminalCapabilities,
								HexApplicationIdentication,
								"0001",//Reserved02,
								HexCardAuthenticatorResultCode,
								HexTerminalType,
								HexInterfaceDeviceSerialNumber,
								HexTransactionCategory,
								HexDedicatedFileName,
								"0001",//HexTerminalIdentificationVersionNumber,
								MyEmvTransaction.SequenceNo,//HexTransactionSequenceNumber,
								HexCVMResult
								);
	
	
	
	sprintf(MyEmvTransaction.Track2,"%s",(char *)HexTrack2Data);
	sprintf(MyEmvTransaction.ICCData,"%s",IDDDataString);
	sprintf(MyEmvTransaction.TransacDateAndTime,"%s",(char *)HexTransactionDate);
	//sprintf(MyEmvTransaction.

	memset(&OutPut,0,sizeof(OutPut));
			
			
	ParseTrackTwo((char *)MyEmvTransaction.Track2,&OutPut,1);
	
	switch(MyEmvTransaction.TransType)
	{
	case PurchaseOnly:
		iRet = Purchase(&MyEmvTransaction);
		if(iRet < 0)
		{
		}
		break;
	}

	
	
	//LOG_PRINTF(("cEMVOnlineProc()"));    
	
	
	if(MyEmvTransaction.cTranResult == 1)
	{
		printEftTranSlip(&MyEmvTransaction);
	}else
	{
		PrintEftError(&MyEmvTransaction);
	}
	
	// initialize output parameters
	
	memcpy(psRspCode, "00", 2);
    memcpy(psAuthCode, "123456", 6);
	*piAuthCodeLen  = 6;
	*piAuthDataLen  = 0;
	*piScriptLen    = 0;
	
			
	SendDebug(OutPut.Pan);
	
	return ONLINE_APPROVE;
	
}


// VISA 512 bits Test Key 90
EMV_CAPK capk_visa_t90={{0xA0,0x00,0x00,0x00,0x03},0x90,
0x01,0x01,64,
{0xC2,0x6B,0x3C,0xB3,0x83,0x3E,0x42,0xD8,0x27,0x0D,0xC1,0x0C,0x89,0x99,0xB2,0xDA,
0x18,0x10,0x68,0x38,0x65,0x0D,0xA0,0xDB,0xF1,0x54,0xEF,0xD5,0x11,0x00,0xAD,0x14,
0x47,0x41,0xB2,0xA8,0x7D,0x68,0x81,0xF8,0x63,0x0E,0x33,0x48,0xDE,0xA3,0xF7,0x80,
0x38,0xE9,0xB2,0x1A,0x69,0x7E,0xB2,0xA6,0x71,0x6D,0x32,0xCB,0xF2,0x60,0x86,0xF1},
1,{0x03},{0x15,0x12,0x31},
{0xB3,0xAE,0x2B,0xC3,0xCA,0xFC,0x05,0xEE,0xEF,0xAA,0x46,0xA2,0xA4,0x7E,0xD5,0x1D,
0xE6,0x79,0xF8,0x23}
};

// VISA 1984 bits Test Key 94
// EMV_CAPK capk_visa_t94={{0xA0,0x00,0x00,0x00,0x03},0x94,
// 0x01,0x01,248,
// {0xD1,0xBE,0x39,0x61,0x5F,0x39,0x5A,0xC9,0x33,0x7E,0x33,0x07,0xAA,0x5A,0x7A,0xC3,
// 0x5E,0xAE,0x00,0x36,0xBF,0x20,0xB9,0x2F,0x9A,0x45,0xD1,0x90,0xB2,0xF4,0x61,0x6A,
// 0xBF,0x9D,0x34,0x0C,0xBF,0x5F,0xBB,0x3A,0x2B,0x94,0xBD,0x8F,0x2F,0x97,0x7C,0x0A,
// 0x10,0xB9,0x0E,0x59,0xD4,0x20,0x1A,0xA3,0x26,0x69,0xE8,0xCB,0xE7,0x53,0xF5,0x36,
// 0x11,0x9D,0xF4,0xFB,0x5E,0x63,0xCE,0xD8,0x7F,0x11,0x53,0xCE,0x91,0x4B,0x12,0x4F,
// 0x3E,0x6B,0x64,0x8C,0xD5,0xC9,0x76,0x55,0xF7,0xAB,0x4D,0xF6,0x26,0x07,0xC9,0x5D,
// 0xA5,0x05,0x17,0xAB,0x8B,0xE3,0x83,0x66,0x72,0xD1,0xC7,0x1B,0xCD,0xE9,0xBA,0x72,
// 0x93,0xFF,0x34,0x82,0xF1,0x24,0xF8,0x66,0x91,0x13,0x0A,0xB0,0x81,0x77,0xB0,0x2F,
// 0x45,0x9C,0x02,0x5A,0x1F,0x3D,0xFF,0xE0,0x88,0x4C,0xE7,0x81,0x22,0x54,0x2E,0xA1,
// 0xC8,0xEA,0x09,0x2B,0x55,0x2B,0x58,0x69,0x07,0xC8,0x3A,0xD6,0x5E,0x0C,0x6F,0x91,
// 0xA4,0x00,0xE4,0x85,0xE1,0x11,0x92,0xAA,0x4C,0x17,0x1C,0x5A,0x1E,0xF5,0x63,0x81,
// 0xF4,0xD0,0x91,0xCC,0x7E,0xF6,0xBD,0x86,0x04,0xCB,0xC4,0xC7,0x4D,0x5D,0x77,0xFF,
// 0xA0,0x7B,0x64,0x1D,0x53,0x99,0x8C,0xDB,0x5C,0x21,0xB7,0xBC,0x65,0xE0,0x82,0xA6,
// 0x51,0x3F,0x42,0x4A,0x4B,0x25,0x2E,0x0D,0x77,0xFA,0x40,0x56,0x98,0x6A,0x0A,0xB0,
// 0xCD,0xA6,0x15,0x5E,0xD9,0xA8,0x83,0xC6,0x9C,0xC2,0x99,0x2D,0x49,0xEC,0xBD,0x47,
// 0x97,0xDD,0x28,0x64,0xFF,0xC9,0x6B,0x8D},
// 3,{0x01,0x00,0x01},{0x15,0x12,0x31},
// {0x1D,0x19,0x31,0x49,0xFD,0x83,0xDC,0xB3,0x37,0xF4,0xA1,0xB5,0xCF,0xAA,0x14,0x4D,
// 0x06,0x6B,0x66,0xFB}
// };

// VISA 1984 bits Test Key 94 20080328
EMV_CAPK capk_visa_t94={{0xA0,0x00,0x00,0x00,0x03},0x94,
0x01,0x01,248,
{},
1,{0x03},{0x15,0x12,0x31},
{0xC4,0xA3,0xC4,0x3C,0xCF,0x87,0x32,0x7D,0x13,0x6B,0x80,0x41,0x60,0xE4,0x7D,0x43,
0xB6,0x0E,0x6E,0x0F}
};

// VISA 1408 bits Test Key 92
EMV_CAPK capk_visa_t92={{0xA0,0x00,0x00,0x00,0x03},0x92,
0x01,0x01,176,
{},
1,{0x03},{0x15,0x12,0x31},
{0x42,0x9C,0x95,0x4A,0x38,0x59,0xCE,0xF9,0x12,0x95,0xF6,0x63,0xC9,0x63,0xE5,0x82,0xED,
0x6E,0xB2,0x53}
};


// VISA 1152 bits Test Key 95
EMV_CAPK capk_visa_t95={{0xA0,0x00,0x00,0x00,0x03},0x95,
0x01,0x01,144,
{},
1,{0x03},{0x15,0x12,0x31},
{0xEE,0x15,0x11,0xCE,0xC7,0x10,0x20,0xA9,0xB9,0x04,0x43,0xB3,0x7B,0x1D,0x5F,0x6E,
0x70,0x30,0x30,0xF6}
};

// VISA 1024 bits Test Key 96
EMV_CAPK capk_visa_t96={{0xA0,0x00,0x00,0x00,0x03},0x96,
0x01,0x01,128,
{},
1,{0x03},{0x15,0x12,0x31},
{0x76,0x16,0xE9,0xAC,0x8B,0xE0,0x14,0xAF,0x88,0xCA,0x11,0xA8,0xFB,0x17,0x96,0x7B,
0x73,0x94,0x03,0x0E}
};

// VISA 768 bits Test Key 97
EMV_CAPK capk_visa_t97={{0xA0,0x00,0x00,0x00,0x03},0x97,
0x01,0x01,96,
{0xAF,0x07,0x54,0xEA,0xED,0x97,0x70,0x43,0xAB,0x6F,0x41,0xD6,0x31,0x2A,0xB1,0xE2,
0x2A,0x68,0x09,0x17,0x5B,0xEB,0x28,0xE7,0x0D,0x5F,0x99,0xB2,0xDF,0x18,0xCA,0xE7,
0x35,0x19,0x34,0x1B,0xBB,0xD3,0x27,0xD0,0xB8,0xBE,0x9D,0x4D,0x0E,0x15,0xF0,0x7D,
0x36,0xEA,0x3E,0x3A,0x05,0xC8,0x92,0xF5,0xB1,0x9A,0x3E,0x9D,0x34,0x13,0xB0,0xD9,
0x7E,0x7A,0xD1,0x0A,0x5F,0x5D,0xE8,0xE3,0x88,0x60,0xC0,0xAD,0x00,0x4B,0x1E,0x06,
0xF4,0x04,0x0C,0x29,0x5A,0xCB,0x45,0x7A,0x78,0x85,0x51,0xB6,0x12,0x7C,0x0B,0x29},
1,{0x03},{0x15,0x12,0x31},
{0x80,0x01,0xCA,0x76,0xC1,0x20,0x39,0x55,0xE2,0xC6,0x28,0x41,0xCD,0x6F,0x20,0x10,
0x87,0xE5,0x64,0xBF}
};

// VISA 896 bits Test Key 98
EMV_CAPK capk_visa_t98={{0xA0,0x00,0x00,0x00,0x03},0x98,
0x01,0x01,112,
{0xCA,0x02,0x6E,0x52,0xA6,0x95,0xE7,0x2B,0xD3,0x0A,0xF9,0x28,0x19,0x6E,0xED,0xC9,
0xFA,0xF4,0xA6,0x19,0xF2,0x49,0x2E,0x3F,0xB3,0x11,0x69,0x78,0x9C,0x27,0x6F,0xFB,
0xB7,0xD4,0x31,0x16,0x64,0x7B,0xA9,0xE0,0xD1,0x06,0xA3,0x54,0x2E,0x39,0x65,0x29,
0x2C,0xF7,0x78,0x23,0xDD,0x34,0xCA,0x8E,0xEC,0x7D,0xE3,0x67,0xE0,0x80,0x70,0x89,
0x50,0x77,0xC7,0xEF,0xAD,0x93,0x99,0x24,0xCB,0x18,0x70,0x67,0xDB,0xF9,0x2C,0xB1,
0xE7,0x85,0x91,0x7B,0xD3,0x8B,0xAC,0xE0,0xC1,0x94,0xCA,0x12,0xDF,0x0C,0xE5,0xB7,
0xA5,0x02,0x75,0xAC,0x61,0xBE,0x7C,0x3B,0x43,0x68,0x87,0xCA,0x98,0xC9,0xFD,0x39},
1,{0x03},{0x15,0x12,0x31},
{0xE7,0xAC,0x9A,0xA8,0xEE,0xD1,0xB5,0xFF,0x1B,0xD5,0x32,0xCF,0x14,0x89,0xA3,0xE5,
0x55,0x75,0x72,0xC1}
};

// VISA 1024 bits Test Key 99
EMV_CAPK capk_visa_t99={{0xA0,0x00,0x00,0x00,0x03},0x99,
0x01,0x01,128,
{},
1,{0x03},{0x15,0x12,0x31},
{0x4A,0xBF,0xFD,0x6B,0x1C,0x51,0x21,0x2D,0x05,0x55,0x2E,0x43,0x1C,0x5B,0x17,0x00,
0x7D,0x2F,0x5E,0x6D}
};

// VISA 1024 bits Live Key 01
EMV_CAPK capk_visa_v01={{0xA0,0x00,0x00,0x00,0x03},0x01,
0x01,0x01,128,
{},
1,{0x03},{0x09,0x12,0x31},
{0xD3,0x4A,0x6A,0x77,0x60,0x11,0xC7,0xE7,0xCE,0x3A,0xEC,0x5F,0x03,0xAD,0x2F,0x8C,
0xFC,0x55,0x03,0xCC}
};

// VISA 896 bits Live Key 03
// EMV_CAPK capk_visa_v03={{0xA0,0x00,0x00,0x00,0x03},0x03,
// 0x01,0x01,112,
// {0xB3,0xE5,0xE6,0x67,0x50,0x6C,0x47,0xCA,0xAF,0xB1,0x2A,0x26,0x33,0x81,0x93,0x50,
// 0x84,0x66,0x97,0xDD,0x65,0xA7,0x96,0xE5,0xCE,0x77,0xC5,0x7C,0x62,0x6A,0x66,0xF7,
// 0x0B,0xB6,0x30,0x91,0x16,0x12,0xAD,0x28,0x32,0x90,0x9B,0x80,0x62,0x29,0x1B,0xEC,
// 0xA4,0x6C,0xD3,0x3B,0x66,0xA6,0xF9,0xC9,0xD4,0x8C,0xED,0x8B,0x4F,0xC8,0x56,0x1C,
// 0x8A,0x1D,0x8F,0xB1,0x58,0x62,0xC9,0xEB,0x60,0x17,0x8D,0xEA,0x2B,0xE1,0xF8,0x22,
// 0x36,0xFF,0xCF,0xF4,0xF3,0x84,0x3C,0x27,0x21,0x79,0xDC,0xDD,0x38,0x4D,0x54,0x10,
// 0x53,0xDA,0x6A,0x6A,0x0D,0x3C,0xE4,0x8F,0xDC,0x2D,0xC4,0xE3,0xE0,0xEE,0xE1,0x5F},
// 1,{0x03},{0x15,0x12,0x31},
// {0xFE,0x70,0xAB,0x3B,0x4D,0x5A,0x1B,0x99,0x24,0x22,0x8A,0xDF,0x80,0x27,0xC7,0x58,
// 0x48,0x3A,0x8B,0x7E}
// };

// VISA 768 bits Live Key 05
// EMV_CAPK capk_visa_v05={{0xA0,0x00,0x00,0x00,0x03},0x05,
// 0x01,0x01,96,
// {0xD0,0x13,0x5C,0xE8,0xA4,0x43,0x6C,0x7F,0x9D,0x5C,0xC6,0x65,0x47,0xE3,0x0E,0xA4,
// 0x02,0xF9,0x81,0x05,0xB7,0x17,0x22,0xE2,0x4B,0xC0,0x8D,0xCC,0x80,0xAB,0x7E,0x71,
// 0xEC,0x23,0xB8,0xCE,0x6A,0x1D,0xC6,0xAC,0x2A,0x8C,0xF5,0x55,0x43,0xD7,0x4A,0x8A,
// 0xE7,0xB3,0x88,0xF9,0xB1,0x74,0xB7,0xF0,0xD7,0x56,0xC2,0x2C,0xBB,0x59,0x74,0xF9,
// 0x01,0x6A,0x56,0xB6,0x01,0xCC,0xA6,0x4C,0x71,0xF0,0x4B,0x78,0xE8,0x6C,0x50,0x1B,
// 0x19,0x3A,0x55,0x56,0xD5,0x38,0x9E,0xCE,0x4D,0xEA,0x25,0x8A,0xB9,0x7F,0x52,0xA3},
// 1,{0x03},{0x15,0x12,0x31},
// {0x86,0xDF,0x04,0x1E,0x79,0x95,0x02,0x35,0x52,0xA7,0x9E,0x26,0x23,0xE4,0x91,0x80,
// 0xC0,0xCD,0x95,0x7A}
// };

// VISA 1152 bits Live Key 07
EMV_CAPK capk_visa_v07={{0xA0,0x00,0x00,0x00,0x03},0x07,
0x01,0x01,144,
{},
1,{0x03},{0x12,0x12,0x31},
{0xB4,0xBC,0x56,0xCC,0x4E,0x88,0x32,0x49,0x32,0xCB,0xC6,0x43,0xD6,0x89,0x8F,0x6F,
0xE5,0x93,0xB1,0x72}
};
//Modified from old 1152 bits key to new one on Feb 6,2004.

// VISA 1408 BITS LIVE KEY 08
EMV_CAPK capk_visa_v08={{0xA0,0x00,0x00,0x00,0x03},0x08, 
0x01,0x01,176, 
{}, 
1,{0x03}, {0x14,0x12,0x31},
{0x20,0xD2,0x13,0x12,0x69,0x55,0xDE,0x20,0x5A,0xDC,0x2F,0xD2,0x82,0x2B,0xD2,0x2D,
0xE2,0x1C,0xF9,0xA8} 
}; 
// expired on December 31, 2014

// VISA 1984 BITS LIVE KEY 09
EMV_CAPK capk_visa_v09={{0xA0,0x00,0x00,0x00,0x03},0x09, 
0x01,0x01,248, 
{}, 
1,{0x03}, {0x16,0x12,0x31},
{0x1F,0xF8,0x0A,0x40,0x17,0x3F,0x52,0xD7,0xD2,0x7E,0x0F,0x26,0xA1,0x46,0xA1,0xC8, 
0xCC,0xB2,0x90,0x46} 
}; 
// expired on December 31, 2016


// Mastercard 896 bits Live Key 02
/*EMV_CAPK capk_mastercard_v02={{0xA0,0x00,0x00,0x00,0x04},0x02,
0x01,0x01,112,
{0xCF,0x42,0x64,0xE1,0x70,0x2D,0x34,0xCA,0x89,0x7D,0x1F,0x9B,0x66,0xC5,0xD6,0x36,
0x91,0xEA,0xCC,0x61,0x2C,0x8F,0x14,0x71,0x16,0xBB,0x22,0xD0,0xC4,0x63,0x49,0x5B,
0xD5,0xBA,0x70,0xFB,0x15,0x38,0x48,0x89,0x52,0x20,0xB8,0xAD,0xEE,0xC3,0xE7,0xBA,
0xB3,0x1E,0xA2,0x2C,0x1D,0xC9,0x97,0x2F,0xA0,0x27,0xD5,0x42,0x65,0xBE,0xBF,0x0A,
0xE3,0xA2,0x3A,0x8A,0x09,0x18,0x7F,0x21,0xC8,0x56,0x60,0x7B,0x98,0xBD,0xA6,0xFC,
0x90,0x81,0x16,0x81,0x6C,0x50,0x2B,0x3E,0x58,0xA1,0x45,0x25,0x4E,0xEF,0xEE,0x2A,
0x33,0x35,0x11,0x02,0x24,0x02,0x8B,0x67,0x80,0x9D,0xCB,0x80,0x58,0xE2,0x48,0x95},
1,{0x03},{0x15,0x12,0x31},
{0xAF,0x1C,0xC1,0xFD,0x1C,0x1B,0xC9,0xBC,0xA0,0x7E,0x78,0xDA,0x6C,0xBA,0x21,0x63,
0xF1,0x69,0xCB,0xB7}
};*/

// Mastercard 1024 bits Live Key 03
EMV_CAPK capk_mastercard_v03={{0xA0,0x00,0x00,0x00,0x04},0x03,
0x01,0x01,128,
{},
1,{0x03},{0x09,0x12,0x31},
{0x5A,0xDD,0xF2,0x1D,0x09,0x27,0x86,0x61,0x14,0x11,0x79,0xCB,0xEF,0xF2,0x72,0xEA,
0x38,0x4B,0x13,0xBB}
};

// Mastercard 1152 bits Live Key 04
EMV_CAPK capk_mastercard_v04={{0xA0,0x00,0x00,0x00,0x04},0x04,
0x01,0x01,144,
{},
1,{0x03},{0x12,0x12,0x31},
{0x38,0x1A,0x03,0x5D,0xA5,0x8B,0x48,0x2E,0xE2,0xAF,0x75,0xF4,0xC3,0xF2,0xCA,0x46,
0x9B,0xA4,0xAA,0x6C}
};
// Mastercard 1408 bits live Key 05
EMV_CAPK capk_mastercard_v05 = {{0xA0,0x00,0x00,0x00,0x04},0x05,
0x01,0x01,176,
{},
1,{0x03},{0x14,0x12,0x31},
{0xEB,0xFA,0x0D,0x5D,0x06,0xD8,0xCE,0x70,0x2D,0xA3,0xEA,0xE8,0x90,0x70,0x1D,0x45,
0xE2,0x74,0xC8,0x45}
};

// Mastercard 1948 bits live Key 06
EMV_CAPK capk_mastercard_v06 = {{0xA0,0x00,0x00,0x00,0x04},0x06,
0x01,0x01,248,
{},
1,{0x03},{0x16,0x12,0x31},
{0xF9,0x10,0xA1,0x50,0x4D,0x5F,0xFB,0x79,0x3D,0x94,0xF3,0xB5,0x00,0x76,0x5E,0x1A,
0xBC,0xAD,0x72,0xD9}
};

// Mastercard 1984 bits transaction time test Key EF
EMV_CAPK capk_mastercard_tEF={{0xA0,0x00,0x00,0x00,0x04},0xEF,
0x01,0x01,248,
{},
1,{0x03},{0x15,0x12,0x31},
{0x21,0x76,0x6E,0xBB,0x0E,0xE1,0x22,0xAF,0xB6,0x5D,0x78,0x45,0xB7,0x3D,0xB4,0x6B,
0xAB,0x65,0x42,0x7A}
};

// Mastercard 1664 bits transaction time test Key F0
EMV_CAPK capk_mastercard_tF0={{0xA0,0x00,0x00,0x00,0x04},0xF0,
0x01,0x01,208,
{},
1,{0x03},{0x15,0x12,0x31},
{0xB8,0xEA,0x49,0x16,0x9B,0x54,0xF3,0xB7,0xFF,0x0D,0xF3,0xA8,0xB6,0x38,0x8C,0x82,
0xA1,0xDB,0xE7,0x30}
};

// Mastercard 1408 bits transaction time test Key F1
EMV_CAPK capk_mastercard_tF1={{0xA0,0x00,0x00,0x00,0x04},0xF1,
0x01,0x01,176,
{},
1,{0x03},{0x15,0x12,0x31},
{0xD8,0xE6,0x8D,0xA1,0x67,0xAB,0x5A,0x85,0xD8,0xC3,0xD5,0x5E,0xCB,0x9B,0x05,0x17,
0xA1,0xA5,0xB4,0xBB}
};

// Mastercard 1152 bits transaction time test Key F2 (exponent=65537)
EMV_CAPK capk_mastercard_tF2={{0xA0,0x00,0x00,0x00,0x04},0xF2,
0x01,0x01,144,
{},
3,{0x01,0x00,0x01},{0x15,0x12,0x31},
{0x0F,0x3B,0xA8,0xCB,0x57,0x77,0xDC,0x4A,0xA9,0x6C,0x30,0xBF,0xB1,0xFC,0x26,0x7A,
0x38,0x2A,0x48,0x47}
};

// Mastercard 1152 bits transaction time test Key F3
EMV_CAPK capk_mastercard_tF3={{0xA0,0x00,0x00,0x00,0x04},0xF3,
0x01,0x01,144,
{},
1,{0x03},{0x15,0x12,0x31},
{0xA6,0x9A,0xC7,0x60,0x3D,0xAF,0x56,0x6E,0x97,0x2D,0xED,0xC2,0xCB,0x43,0x3E,0x07,
0xE8,0xB0,0x1A,0x9A}
};

// Mastercard 896 bits transaction time test Key F4
EMV_CAPK capk_mastercard_tF4={{0xA0,0x00,0x00,0x00,0x04},0xF4,
0x01,0x01,112,
{0x9C,0xFA,0xD5,0x4B,0x40,0x29,0x7C,0x1C,0xDE,0x23,0xFC,0xB3,0xEF,0x68,0xD3,0x18,
0x34,0x1A,0x47,0x27,0xAE,0x1D,0xAA,0x2B,0xEB,0xE3,0x58,0x72,0xEF,0x3D,0xC9,0x07,
0x46,0x29,0x7B,0x06,0x6E,0xD1,0xCE,0x3C,0x07,0xC1,0xF2,0x34,0xFF,0x54,0x90,0x42,
0x5E,0x8B,0x14,0x67,0x4C,0xC5,0x7E,0x43,0x97,0xA5,0x15,0x84,0xFF,0x5E,0xBA,0x6B,
0x5D,0x54,0xD9,0x9D,0x2C,0x9F,0xC9,0x9D,0x5E,0x4C,0xAC,0xB3,0x48,0x7A,0xBA,0x79,
0x0F,0x28,0xE3,0x04,0x98,0x7A,0xFA,0x7F,0x5F,0x92,0xE2,0x2D,0x89,0xFF,0x51,0x0C,
0x1B,0x58,0x19,0x41,0x16,0x6C,0x7C,0xCB,0x11,0xEF,0xB0,0x8D,0xE6,0x07,0x46,0x0D},
1,{0x03},{0x15,0x12,0x31},
{0x98,0xCE,0xB9,0xE0,0xE8,0xED,0x52,0xAB,0xDD,0x85,0x49,0xFD,0x50,0xAC,0xEC,0xA3,
0xBF,0x51,0xA7,0x86}
};

// Mastercard 1024 bits transaction time test Key F8 (also for ETEC test)
EMV_CAPK capk_mastercard_tF8={{0xA0,0x00,0x00,0x00,0x04},0xF8,
0x01,0x01,128,
{},
1,{0x03},{0x15,0x12,0x31},
{0xF0,0x6E,0xCC,0x6D,0x2A,0xAE,0xBF,0x25,0x9B,0x7E,0x75,0x5A,0x38,0xD9,0xA9,0xB2,
0x4E,0x2F,0xF3,0xDD}
};

EMV_CAPK capk_mastercard_tFA = {{0xA0,0x00,0x00,0x00,0x04},0xFA,
0x01,0x01,144,
{},
1,{0x03},{0x15,0x12,0x31},
{0x5B,0xED,0x40,0x68,0xD9,0x6E,0xA1,0x6D,0x2D,0x77,0xE0,0x3D,0x60,0x36,0xFC,0x7A,
0x16,0x0E,0xA9,0x9C}
};


// Mastercard - eurocheque 1024 bits transaction time test Key F8(also for ETEC test)
EMV_CAPK capk_eurocheque_tF8={{0xA0,0x00,0x00,0x00,0x10},0xF8,
0x01,0x01,128,
{},
1,{0x03},{0x15,0x12,0x31},
{0x0F,0x44,0x88,0x56,0x8C,0xF8,0xAF,0xB0,0xC1,0x2E,0xC6,0x53,0xCF,0x7A,0x04,0xD2,
0xD4,0x6D,0xCD,0x34}
};

// Mastercard 1024 bits Test Key FE(also for ETEC test)
EMV_CAPK capk_mastercard_tFE={{0xA0,0x00,0x00,0x00,0x04},0xFE,
0x01,0x01,128,
{},
1,{0x03},{0x15,0x12,0x31},
{0x9A,0x29,0x5B,0x05,0xFB,0x39,0x0E,0xF7,0x92,0x3F,0x57,0x61,0x8A,0x9F,0xDA,0x29,
0x41,0xFC,0x34,0xE0}
};

// Mastercard - eurocheque 1024 bits Test Key FE(also for ETEC test)
EMV_CAPK capk_eurocheque_tFE={{0xA0,0x00,0x00,0x00,0x10},0xFE,
0x01,0x01,128,
{},
1,{0x03},{0x15,0x12,0x31},
{0x2C,0xFB,0xB8,0x24,0x09,0xED,0x86,0xA3,0x19,0x73,0xB0,0xE0,0xCE,0xEA,0x38,0x1B,
0xC4,0x3C,0x80,0x97}
};

// JCB 1024 bits Test Key 08
EMV_CAPK capk_jcb_t08={{0xA0,0x00,0x00,0x00,0x65},0x08,
0x01,0x01,128,
{},
1,{0x03},{0x15,0x12,0x31},
{0xDD,0x36,0xD5,0x89,0x62,0x28,0xC8,0xC4,0x90,0x07,0x42,0xF1,0x07,0xE2,0xF9,0x1F,
0xE5,0x0B,0xC7,0xEE}
};

// JCB 1024 bits Test Key 01
EMV_CAPK capk_jcb_t01={{0xF1,0x23,0x45,0x67,0x89},0x01,
0x01,0x01,128,
{},
1,{0x03},{0x15,0x12,0x31},
{0xCC,0xC1,0x8B,0x93,0x7C,0x85,0x9F,0x42,0x9C,0x33,0xA3,0xDB,0xEB,0x45,0xE3,0x1E,
0x17,0x5B,0x8F,0xD1}
};

// JCB 1024 bits Test Key 02,for JCB2.0 test
EMV_CAPK capk_jcb_t02={{0xF1,0x23,0x45,0x67,0x89},0x02,
0x01,0x01,128,
{},
3,{0x01,0x00,0x01},{0x15,0x12,0x31},
{0x5A,0x52,0xA2,0x86,0x9A,0x93,0x8F,0x82,0xBE,0x41,0xA1,0x80,0xA3,0x98,0x09,0x55,
0x46,0xD4,0xF6,0xAA}
};

// JCB 1152 bits Test Key 03,for JCB2.0 test
EMV_CAPK capk_jcb_t03={{0xF1,0x23,0x45,0x67,0x89},0x03,
0x01,0x01,144,
{},
1,{0x03},{0x15,0x12,0x31},
{0xE9,0x6C,0x1B,0x32,0xB4,0xBA,0xEE,0xDA,0x02,0x99,0x91,0x51,0x42,0x23,0x63,0x72,
0xFF,0xC6,0x8A,0x64}
};

// for JCB test [7/31/2006 Tommy] ---->
EMV_CAPK capk_jcb_tDA={{0xA0,0x00,0x00,0x00,0x65},0xDA,
0x01,0x01,128,
{},
1,{0x03}, {0x10,0x10,0x31},
{0x5C,0x03,0x7E,0x18,0x15,0xE3,0x8B,0x79,0xD4,0xD5,0xC1,0x5F,0xF9,0x0C,0xE4,0xE3,0xF2,
0x07,0x08,0x63}
};

EMV_CAPK capk_jcb_tEA={{0xA0,0x00,0x00,0x00,0x65},0xEA,
0x01,0x01,144,
{},
1,{0x03}, {0x10,0x12,0x31},
{0x88,0xF6,0xD0,0x4B,0xED,0x35,0xD6,0xB4,0x3E,0x69,0x7D,0x76,0x5A,0xE3,0x29,0x3F,0x64,
0x9A,0x96,0x1F}
};

EMV_CAPK capk_jcb_tEB={{0xA0,0x00,0x00,0x00,0x65},0xEB,
0x01,0x01,176,
{},
1,{0x03},{0x10,0x10,0x31},
{0xA1,0x9C,0xAF,0x70,0x06,0x03,0xE3,0x7B,0xC7,0xC2,0xE5,0xC4,0x20,0xA1,0x1A,0x56,0xD1,
0xA8,0xBA,0xC3}
};

EMV_CAPK capk_jcb_tEC={{0xA0,0x00,0x00,0x00,0x65},0xEC,
0x01,0x01,248,
{},
1,{0x03},{0x10,0x10,0x31},
{0xA1,0x59,0x45,0x94,0x69,0x35,0x95,0x68,0x45,0xAD,0xB8,0xAB,0xE7,0x3E,0x5B,0x0B,0xEE,
0xF7,0x6E,0xCB}
};

EMV_CAPK capk_jcb_t0F={{0xA0,0x00,0x00,0x00,0x65},0x0F,
0x01,0x01,144,
{/*0x9F*/0x9E,},
1,{0x03},{0x10,0x12,0x31},
{0x2A,0x1B,0x82,0xDE,0x00,0xF5,0xF0,0xC4,0x01,0x76,0x0A,0xDF,0x52,0x82,0x28,0xD3,0xED,
0xE0,0xF4,0x03}
};
// for JCB test [7/31/2006 Tommy] <----

// JCB Production keys [7/31/2006 Tommy] --------->
/************************************************
以下CAPK 为实际生产CAPK
************************************************/
EMV_CAPK capk_jcb_t09={{0xA0,0x00,0x00,0x00,0x65},0x09,
0x01,0x01,128,
{},
1,{0x03},{0x09,0x12,0x31},
{0x44,0x10,0xC6,0xD5,0x1C,0x2F,0x83,0xAD,0xFD,0x92,0x52,0x8F,0xA6,0xE3,0x8A,0x32,0xDF,
0x04,0x8D,0x0A}
};

EMV_CAPK capk_jcb_t10={{0xA0,0x00,0x00,0x00,0x65},0x10,
0x01,0x01,144,
{},
1,{0x03},{0x12,0x12,0x31},
{0xC7,0x5E,0x52,0x10,0xCB,0xE6,0xE8,0xF0,0x59,0x4A,0x0F,0x19,0x11,0xB0,0x74,0x18,
0xCA,0xDB,0x5B,0xAB}
};

EMV_CAPK capk_jcb_t12={{0xA0,0x00,0x00,0x00,0x65},0x12,
0x01,0x01,176,
{},
1,{0x03},{0x14,0x12,0x31},
{0x87,0x4B,0x37,0x9B,0x7F,0x60,0x7D,0xC1,0xCA,0xF8,0x7A,0x19,0xE4,0x00,0xB6,0xA9,
0xE2,0x51,0x63,0xE8}
};

EMV_CAPK capk_jcb_t14={{0xA0,0x00,0x00,0x00,0x65},0x14,
0x01,0x01,248,
{},
1,{0x03},{0x16,0x12,0x31},
{0xC0,0xD1,0x5F,0x6C,0xD9,0x57,0xE4,0x91,0xDB,0x56,0xDC,0xDD,0x1C,0xA8,0x7A,0x03,
0xEB,0xE0,0x6B,0x7B}
};
// JCB Production keys [7/31/2006 Tommy] <---------

EMV_CAPK capk_amex_live03={{0xA0, 0x00, 0x00, 0x00, 0x25},0x03,
0x01,0x01,0x80,
{},
1,{0x03},{0x15,0x12,0x31},
{0x87, 0x08, 0xA3, 0xE3, 0xBB, 0xC1, 0xBB, 0x0B, 0xE7, 0x3E, 0xBD, 0x8D, 0x19, 0xD4, 0xE5, 0xD2,
0x01, 0x66, 0xBF, 0x6C}
};

EMV_CAPK capk_amex_test62={{0xA0, 0x00, 0x00, 0x00, 0x25},0x62,
0x01,0x01,0x60,
{0xBA, 0x29, 0xDE, 0x83, 0x09, 0x0D, 0x8D, 0x5F, 0x4D, 0xFF, 0xCE, 0xB9, 0x89, 0x18, 0x99, 0x5A,
0x76, 0x8F, 0x41, 0xD0, 0x18, 0x3E, 0x1A, 0xCA, 0x3E, 0xF8, 0xD5, 0xED, 0x90, 0x62, 0x85, 0x3E,
0x40, 0x80, 0xE0, 0xD2, 0x89, 0xA5, 0xCE, 0xDD, 0x4D, 0xD9, 0x6B, 0x1F, 0xEA, 0x2C, 0x53, 0x42,
0x84, 0x36, 0xCE, 0x15, 0xA2, 0xA1, 0xBF, 0xE6, 0x9D, 0x46, 0x19, 0x7D, 0x3F, 0x5A, 0x79, 0xBC,
0xF8, 0xF4, 0x85, 0x8B, 0xFF, 0xA0, 0x4E, 0xDB, 0x07, 0xFC, 0x5B, 0xE8, 0x56, 0x0D, 0x9C, 0xE3,
0x8F, 0x5C, 0x3C, 0xA3, 0xC7, 0x42, 0xED, 0xFD, 0xBA, 0xE3, 0xB5, 0xE6, 0xDD, 0xA4, 0x55, 0x57},
1,{0x03},{0x15,0x12,0x31},
{0xCC, 0xC7, 0x30, 0x3F, 0xF2, 0x95, 0xA9, 0xF3, 0x5B, 0xA6, 0x1B, 0xD3, 0x1E, 0x27, 0xEA, 0xBD, 
0x59, 0x65, 0x82, 0x65}
};

EMV_CAPK capk_amex_test64={{0xA0, 0x00, 0x00, 0x00, 0x25},0x64,
0x01,0x01,0x80,
{},
1,{0x03},{0x15,0x12,0x31},
{0x79, 0x2B, 0x12, 0x1D, 0x86, 0xD0, 0xF3, 0xA9, 0x95, 0x82, 0xDB, 0x06, 0x97, 0x44, 0x81, 0xF3,
0xB2, 0xE1, 0x84, 0x54}
};

EMV_CAPK capk_amex_test65={{0xA0, 0x00, 0x00, 0x00, 0x25},0x65,
0x01,0x01,0x90,
{},
1,{0x03},{0x15,0x12,0x31},
{0x89, 0x4C, 0x5D, 0x08, 0xD4, 0xEA, 0x28, 0xBB, 0x79, 0xDC, 0x46, 0xCE, 0xAD, 0x99, 0x8B, 0x87,
 0x73, 0x22, 0xF4, 0x16}
};

EMV_CAPK capk_amex_test66={{0xA0, 0x00, 0x00, 0x00, 0x25},0x66,
0x01,0x01,0x80,
{},
1,{0x03},{0x15,0x12,0x31},
{0xF3, 0x67, 0xCB, 0x70, 0xF9, 0xC9, 0xB6, 0x7B, 0x58, 0x0F, 0x53, 0x38, 0x19, 0xE3, 0x02, 0xBA,
 0xC0, 0x33, 0x00, 0x90}
};

EMV_CAPK capk_amex_test04={{0xA0, 0x00, 0x00, 0x00, 0x25},0x04,
0x01,0x01,0x60,
{0xD0, 0xF5, 0x43, 0xF0, 0x3F, 0x25, 0x17, 0x13, 0x3E, 0xF2, 0xBA, 0x4A, 0x11, 0x04, 0x48, 0x67,
0x58, 0x63, 0x0D, 0xCF, 0xE3, 0xA8, 0x83, 0xC7, 0x7B, 0x4E, 0x48, 0x44, 0xE3, 0x9A, 0x9B, 0xD6,
0x36, 0x0D, 0x23, 0xE6, 0x64, 0x4E, 0x1E, 0x07, 0x1F, 0x19, 0x6D, 0xDF, 0x2E, 0x4A, 0x68, 0xB4,
0xA3, 0xD9, 0x3D, 0x14, 0x26, 0x8D, 0x72, 0x40, 0xF6, 0xA1, 0x4F, 0x0D, 0x71, 0x4C, 0x17, 0x82,
0x7D, 0x27, 0x9D, 0x19, 0x2E, 0x88, 0x93, 0x1A, 0xF7, 0x30, 0x07, 0x27, 0xAE, 0x9D, 0xA8, 0x0A,
0x3F, 0x0E, 0x36, 0x6A, 0xEB, 0xA6, 0x17, 0x78, 0x17, 0x17, 0x37, 0x98, 0x9E, 0x1E, 0xE3, 0x09},
1,{0x03},{0x15,0x12,0x31},
{0xFD, 0xD7, 0x13, 0x9E, 0xC7, 0xE0, 0xC3, 0x31, 0x67, 0xFD, 0x61, 0xAD, 0x3C, 0xAD, 0xBD,
0x68, 0xD6, 0x6E, 0x91, 0xC5}
};



EMV_CAPK capk_verve_t04={{0xA0,0x00,0x00,0x03,0x71},0x04,
0x01,0x01,144,
{},
1,{0x03},{0x17,0x12,0x31},
{0x8B,0x36,0xA3,0xE3,0xD8,0x14,0xCE,0x6C,0x6E,0xBE,0xAA,0xF2,0x76,0x74,0xBB,0x7B,0xC6,0x72,0x75,0xB1}
};



EMV_CAPK capk_verve_t03={{0xA0,0x00,0x00,0x03,0x71},0x03,
0x01,0x01,144,
{},
1,{0x03},{0x17,0x12,0x31},
{0x31, 0x9F, 0x3C, 0x60, 0x8B, 0x67, 0xF1, 0x11, 0x8C, 0x72, 0x9B, 0x0E, 0x15, 0x16, 0xEA, 0xB0, 0x7C, 0xB2, 0x90, 0xC8}
};
struct{
	unsigned char KeyID;
	EMV_CAPK *KeyValue;
}AllEmvKey[]={
{0x90,&capk_visa_t90},
{0x92,&capk_visa_t92},
{0x94,&capk_visa_t94},
{0x95,&capk_visa_t95},
{0x96,&capk_visa_t96},
//{0x97,&capk_visa_t97},
{0x98,&capk_visa_t98},
{0x99,&capk_visa_t99},
{0x01,&capk_visa_v01},
//{0x03,&capk_visa_v03},
//{0x05,&capk_visa_v05},
{0x07,&capk_visa_v07},
{0x08,&capk_visa_v08},
{0x09,&capk_visa_v09},
//{0x02,&capk_mastercard_v02},
{0x03,&capk_mastercard_v03},
{0x04,&capk_mastercard_v04},
{0x05,&capk_mastercard_v05},
{0x06,&capk_mastercard_v06},
{0xEF,&capk_mastercard_tEF},
{0xF0,&capk_mastercard_tF0},
{0xF1,&capk_mastercard_tF1},
{0xF2,&capk_mastercard_tF2},
{0xF3,&capk_mastercard_tF3},
{0xF4,&capk_mastercard_tF4},
{0xF8,&capk_mastercard_tF8},
{0xFA,&capk_mastercard_tFA},
{0xF8,&capk_eurocheque_tF8},
{0xFE,&capk_mastercard_tFE},
{0xFE,&capk_eurocheque_tFE},
// {0x08,&capk_jcb_t08},
// {0x01,&capk_jcb_t01},
// {0x02,&capk_jcb_t02},
// {0x03,&capk_jcb_t03},

// JCB test [7/31/2006 Tommy] ------->
{0xEB,&capk_jcb_tEB},
{0xEC,&capk_jcb_tEC},
{0x0f,&capk_jcb_t0F},
{0xDA,&capk_jcb_tDA},
{0xEA,&capk_jcb_tEA},
// JCB test [7/31/2006 Tommy] <-------

{0x03,&capk_amex_live03},
{0x62,&capk_amex_test62},
{0x64,&capk_amex_test64},
{0x65,&capk_amex_test65},
{0x66,&capk_amex_test66},
{0x04,&capk_amex_test04},

{0x04,&capk_verve_t04},
{0x03,&capk_verve_t03}
};

//EMV用于定义
EMV_APPLIST EMV_TEST_APP={
	"",
	"\xA0\x00\x00\x00\x99\x90\x90",
	7,
	PART_MATCH,
	0,
	0,
	0,
	1,
	1,
	1,
	//20000,0,
	0,
	0,
	"\x00\x10\x00\x00\x00",
	"\xD8\x40\x04\xF8\x00",
	"\xD8\x40\x00\xA8\x00",
	"\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x8c",
	""
};

EMV_APPLIST JCB_TEST_APP={
	"","\xF1\x23\x45\x67\x89\x01\x23",
	7,PART_MATCH,0,0,0,1,1,1,
	20000,0,
	"\x00\x10\x00\x00\x00",
	"\xD8\x40\x04\xF8\x00",
	"\xD8\x40\x00\xA8\x00",
	"\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x8c",
	"",
};


EMV_APPLIST JCB_TEST_APP1={	// add for JCB testing [7/31/2006 Tommy]
	"","\xA0\x00\x00\x00\x65\x10\x10",
		7,PART_MATCH,0,0,0,1,1,1,
		500,0,
		"\x00\x10\x00\x00\x00",
		"\xFC\x60\xAC\xF8\x00",
		"\xFC\x60\x24\xA8\x00",
		"\x00\x00\x12\x34\x56",
		"\x03\x9F\x37\x04",
		"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
		"\x02\x00",
		"",
};

EMV_APPLIST VSDC_TEST_APP={
	"","\xA0\x00\x00\x00\x03\x10\x10",
	7,PART_MATCH,0,0,0,1,1,1,
	//20000,0,
	100,1000,
	"\x00\x10\x00\x00\x00",
	"\xD8\x40\x04\xF8\x00",
	"\xD8\x40\x00\xA8\x00",
	"\x00\x00\x12\x34\x56",
	"\x03\x9F\x37\x04",
	"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
	"\x00\x8c",
	"",
};

EMV_APPLIST VSDC_TEST_APP1={
	"","\xA0\x00\x00\x00\x03\x20\x10",
		7,PART_MATCH,0,0,0,1,1,1,
		//20000,0,
		100,1000,
		"\x00\x10\x00\x00\x00",
		"\xD8\x40\x04\xF8\x00",
		"\xD8\x40\x00\xA8\x00",
		"\x00\x00\x12\x34\x56",
		"\x03\x9F\x37\x04",
		"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
		"\x00\x8c",
		"",
};

EMV_APPLIST AMEX_TEST_APP={
	"","\xA0\x00\x00\x00\x25\x01",
		6,PART_MATCH,0,0,0,1,1,1,
		20000,0,
		"\x00\x00\x00\x00\x00",
		"\x00\x00\x00\x00\x00",
		"\x00\x00\x00\x00\x00",
		"\x00\x00\x12\x34\x56",
		"\x03\x9F\x37\x04",
		"\x9F\x37\x04",
		"\x00\x01",
		"",
};

EMV_APPLIST MCC_APP1={
	"",
		"\xA0\x00\x00\x00\x04\x10\x10",
		7, 
		PART_MATCH,0,0,0,1,1,1,
//		20000,0,
//		500,0,   
        0,0,
		"\x04\x00\x00\x00\x00",
		"\xF8\x50\xAC\xF8\x00",
		"\xFC\x50\xAC\xA0\x00",
		"\x00\x00\x12\x34\x56",
		"\x03\x9F\x37\x04",
		"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
		"\x00\x02",
		"",
}; 

EMV_APPLIST MCC_APP2={
	"",
		"\xA0\x00\x00\x00\x04\x30\x60",
		7, /*7->5*/
		PART_MATCH,0,0,0,1,1,1,
//		20000,0,
// 		500,0,
		0,0,
		"\x04\x00\x00\x00\x00",
		"\xF8\x50\xAC\xF8\x00",
		"\xFC\x50\xAC\xA0\x00",
		"\x00\x00\x12\x34\x56",
		"\x03\x9F\x37\x04",
		"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
		"\x00\x02",
		"",
};

EMV_APPLIST MCC_APP3={
	"",
		"\xA0\x00\x00\x00\x04\x60\x00",
		7, /*7->5*/
		PART_MATCH,0,0,0,1,1,1,
//		20000,0,
// 		500,0,
		0,0,
		"\x04\x00\x00\x00\x00",
		"\xF8\x50\xAC\xF8\x00",
		"\xFC\x50\xAC\xA0\x00",
		"\x00\x00\x12\x34\x56",
		"\x03\x9F\x37\x04",
		"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
		"\x00\x02",
		"",
};

EMV_APPLIST MCC_APP4={
	"",
		"\xA0\x00\x00\x00\x04\x60\x10",
		7, /*7->5*/
		PART_MATCH,0,0,0,1,1,1,
//		20000,0,
// 		500,0,
        0,0,
		"\x04\x00\x00\x00\x00",
		"\xF8\x50\xAC\xF8\x00",
		"\xFC\x50\xAC\xA0\x00",
		"\x00\x00\x12\x34\x56",
		"\x03\x9F\x37\x04",
		"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
		"\x00\x02",
		"",
};

EMV_APPLIST MCC_APP5={
	"",
		"\xA0\x00\x00\x00\x10\x10\x30",
		7, /*7->5*/
		PART_MATCH,0,0,0,1,1,1,
//		20000,0,
// 		500,0,
        0,0,
		"\x04\x00\x00\x00\x00",
		"\xF8\x50\xAC\xF8\x00",
		"\xFC\x50\xAC\xA0\x00",
		"\x00\x00\x12\x34\x56",
		"\x03\x9F\x37\x04",
		"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04",
		"\x00\x02",
		"",
};


EMV_APPLIST VERVE_APP={
	"",										//AppName
		"\xA0\x00\x00\x03\x71\x00\x01",		//AID
		7, 							//AidLen
		PART_MATCH,0,0,0,1,1,1,				//SelFlag,Priority,TargetPer,MaxTargetPer,FloorLimitCheck,RandTransSel,VelocityCheck
//		20000,0,
// 		500,0,
        0,0,								//FloorLimit,Threshold
		"\x04\x00\x00\x00\x00",				//ActDenial
		"\xF8\x50\xAC\xF8\x00",				//ActOnline
		"\xFC\x50\xAC\xA0\x00",				//ActDefault
		"\x00\x00\x12\x34\x56",				//AcquierId
		"\x03\x9F\x37\x04",					//dDol
		"\x0F\x9F\x02\x06\x5F\x2A\x02\x9A\x03\x9C\x01\x95\x05\x9F\x37\x04", //tDol
		"\x00\x02",							//Version
		"",									//RiskManData
};

void InitTestKeys(void)
{
	int		i;

	//LOG_PRINTF(("InitTestKeys"));

	for(i=0; i<sizeof(AllEmvKey)/sizeof(AllEmvKey[0]); i++)
	{
		EMVAddCAPK(AllEmvKey[i].KeyValue);
	}
}

void InitTestApps(void)
{
	//LOG_PRINTF(("InitTestApps"));

	EMVAddApp(&EMV_TEST_APP);
	EMVAddApp(&JCB_TEST_APP1);
//	EMVAddApp(&JCB_TEST_APP);
	EMVAddApp(&VSDC_TEST_APP1);
	EMVAddApp(&VSDC_TEST_APP);
	EMVAddApp(&AMEX_TEST_APP);
	EMVAddApp(&MCC_APP1);
	EMVAddApp(&MCC_APP2);
	EMVAddApp(&MCC_APP3);
	EMVAddApp(&MCC_APP4);
	EMVAddApp(&MCC_APP5);
	EMVAddApp(&VERVE_APP);
}

