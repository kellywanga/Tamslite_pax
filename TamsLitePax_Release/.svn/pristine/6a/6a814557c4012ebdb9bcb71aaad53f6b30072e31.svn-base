...
typedef struct {
	...
	unsigned char HashInd;   //Hashindex        //HASH算法标志
	unsigned char ArithInd;  //ArithIdshldbe 1         //RSA算法标志
	unsigned char ModulLen;   //strlen/2       //模长度
	unsigned char Modul[248];        //模
	unsigned char ExponentLen;       //指数长度
	unsigned char Exponent[3];       //指数
	unsigned char ExpDate[3];  //expri      //有效期(YYMMDD)
